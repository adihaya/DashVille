...
shortcutsPage.init = function() {
  
  var searchBox = createSearchBox({
    searchOnEnter: shortcutsPage.openFirstShortcut,
    searchOnInput: shortcutsPage.searchServices,
    buttonType: "launch",
  ...
  document.getElementById("shortcutsPage").appendChild(searchBox);
  
  shortcutsPage.content = createContent({
    "toolbarCount": 1  
  });
  document.getElementById("shortcutsPage").appendChild(shortcutsPage.content);
  
  shortcutsPage.loadList(); 
};

shortcutsPage.createContent = function(){
  var content = document.createElement("div");
  content.dataset.toolbarCount = 1;
  content.className = "content";
  loadScrollShadow(content); //from main.js
  return content;
};

shortcutsPage.openFirstShortcut = function(){
  document.querySelector("#shortcutsPage .buttonList>li:not(.off)").click();
};

shortcutsPage.searchServices = function() {
  var query = document.querySelector("#shortcutsPage .searchInput").value;
  
  var shortcuts = document.querySelectorAll("#shortcutsPage .content li");
  [].forEach.call(shortcuts, function(element){
    if(shortcutsPage.services[element.dataset.id].name.toLowerCase().indexOf(query.toLowerCase()) == -1){
...
shortcutsPage.loadList = function(query) {
  var ul = [];
  for(var i in shortcutsPage.services){
    shortcutsPage.loadService(i,ul,query);
  };
  shortcutsPage.sortList(ul);
};

shortcutsPage.refreshList = function(listItems) {
  shortcutsPage.content.textContent = "";
  
  if(listItems.length == 0){
    shortcutsPage.content.appendChild(createCardHeader({"message": "No results"}));
  }
  else{
    var list = document.createElement("div");
    list.className = "card buttonList";
    for(var i=0;i<listItems.length;i++){
      list.appendChild(listItems[i]);
    };
    shortcutsPage.content.appendChild(list);
  };
};

shortcutsPage.loadService = function(id,ul,query) {
  var serviceName = shortcutsPage.services[id].name;
  var properties = {
    "id": id,
    "name": serviceName,
    "imgName": shortcutsPage.services[id].icon == "" ? "undefined": shortcutsPage.services[id].icon,
    "tooltip": (shortcutsPage.services[id].tooltip) ? (serviceName + " - " + shortcutsPage.services[id].tooltip) : serviceName,
    "linkUrl": shortcutsPage.services[id].url,
    "clickFunction": shortcutsPage.openShortcut
  };
  if(query){
    if(serviceName.toLowerCase().indexOf(query.toLowerCase()) == -1){
      return;
    };
  };
...
shortcutsPage.openShortcut = function() {
  ...
  chrome.storage.sync.get("serviceClicks", function(item) {
    ...
    var url = document.querySelector('#shortcutsPage [data-id="' + serviceId + '"]').dataset.link;
    window.open(url, "_blank");
  });
};

shortcutsPage.focusSearchbox = function(){
  var element = document.querySelector("#shortcutsPage .searchInput");
  focusSearchBox(element);
};

shortcutsPage.sortList = function(list) {
  var newList = [];
  chrome.storage.sync.get("serviceClicks", function(item) {
    ...
    shortcutsPage.refreshList(newList);
...
if(document.body){
  ...
  window.addEventListener("DOMContentLoaded", shortcutsPage.init);
};