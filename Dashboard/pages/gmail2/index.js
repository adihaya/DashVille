var gmailPage = {};
gmailPage.iframeUrl = "https://mail.google.com/mail/mu/mp/?mui=blackMenu&hl=" + browserLang + "#tl";

gmailPage.init = function() {
  gmailPage.body = document.getElementById("gmailPage");
  var searchBox = createSearchBox({
    "searchOnEnter": gmailPage.loadSearchResults,
    "inputPmsg": 4
  });
  gmailPage.body.appendChild(searchBox);
  gmailPage.searchInput = searchBox.querySelector("input");
  
  gmailPage.iframe = createContent({
    "toolbarCount": 2,
    "iframeSrc": gmailPage.iframeUrl
  });
  gmailPage.body.appendChild(gmailPage.iframe);
  
  var linksSection = createLinksSection({"onMouseEnter": gmailPage.loadLinksSection, "name": i18n("msg189")});
  gmailPage.body.appendChild(linksSection);
  
  gmailPage.gadgetMessageListener();
};

gmailPage.loadSearchResults = function(query) {
  gmailPage.iframe.src = gmailPage.iframeUrl + "#tl/search/" + query;
};
...
gmailPage.gadgetMessageListener = function(){
  chrome.extension.onMessage.addListener(
    function(request, sender, sendResponse) {
      ...
      if (request.iframeUrl){
        sendResponse({farewell: gmailPage.iframeUrl = request.iframeUrl});
      };
    }
  );
};	
...
gmailPage.linksSectionShortcutClick = function() {
  if(gmailPage.searchInput.value.length > 0) {
    window.open(this.dataset.search.replace("[browserLang]",browserLang).replace("[query]",gmailPage.searchInput.value))
  }
  else{
